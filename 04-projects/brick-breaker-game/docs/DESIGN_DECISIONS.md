...
**분리 기준**:
1. ✅ **관심사 분리**: 입력/로직/애니메이션
2. ✅ **실행 조건**: 애니메이션은 항상, 입력/로직은 게임 중만
3. ✅ **책임 명확화**: 각 함수가 하나의 역할만

**장점**:
- ✅ 가독성: 각 함수 이름이 역할 명확히 표현
- ✅ 유지보수: 수정 시 해당 함수만 찾으면 됨
- ✅ 테스트: 각 함수를 독립적으로 테스트 가능
- ✅ 확장: 새 로직 추가 시 적절한 함수에 추가

**단점**:
- ❌ 코드 길이 증가: 829 → 849줄 (+20줄)
- ❌ 함수 호출 오버헤드 (미미함)
...
- **최초 작성일**: 2025-11-13
- **최종 업데이트**: 2025-11-25 (Stage 24 추가)
- **프로젝트**: 벽돌깨기 게임
