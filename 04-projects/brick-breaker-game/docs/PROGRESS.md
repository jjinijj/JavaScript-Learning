...
### 1. Stage 25 스킵 결정 (2025-11-26)
**공 상태 패턴 (State Pattern) 적용 → 스킵**

**스킵 이유**:
- `launched` 플래그가 사용되는 곳은 단 3곳 (update, launch, checkPaddleCollision)
- State Pattern 도입 시 4개 파일 + 70줄 이상 코드로 복잡도 폭증
- YAGNI 원칙: 현재 공의 상태는 2개뿐 (패들에 붙음 vs 날아감)
- 단순 조건문이 더 명확하고 유지보수 쉬움

**BallManager 검토 결과**:
- **필요한 경우**: 공 분리 아이템 (3개로 분할) 100% 추가 예정일 때
- **활용 시나리오**: 공 회수/재발사, 공별 독립 효과, 점진적 난이도, 통계 추적, 공 합체 등
- **결론**: 현재는 과도한 사전 설계 (오버 엔지니어링)
- **대안**: 공 분리 아이템이 실제로 필요해지면 그때 BallManager 추가

**설계 원칙**: 애자일 방식 - "지금 필요하지 않으면 만들지 마라"
